    ...
    for (int i = 0; i < gameBoard->width; i++)
{
    ...
    char str[10];
    sprintf(str, "%d", gameSettings.score);
    renderText(renderer, font, str, TILE_SIZE * 15, TILE_SIZE * 15);

    renderText(renderer, font, "Score : ", TILE_SIZE * (BOARD_WIDTH + 1), 1);
    renderText(renderer, font, "1", TILE_SIZE * (BOARD_WIDTH + 5), 1);
...
void render(SDL_Renderer *renderer, SDL_Window *window, SHAPE activeShape, SHAPE nextShape, GameBoard *gameBoard, GameState gameState, int selectedMenuItem, TTF_Font *font,GameSettings gameSettings)
{
    ...
    case PLAYING:
        renderGrid(renderer);
        draw(activeShape, nextShape, renderer, gameBoard, font,gameSettings);
        break;
    case SETTINGS:
        break;
    case QUIT_GAME:
